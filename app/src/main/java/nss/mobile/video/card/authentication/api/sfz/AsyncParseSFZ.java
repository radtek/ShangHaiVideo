                ...
                case READ_SFZ:
                    C.sTHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (resultSFZ == null) {
                                AsyncParseSFZ.this.obtainMessage(FIND_CARD_FAIL, ParseSFZAPI.Result.FIND_FAIL, -1).sendToTarget();
                                return;
                            }
    ...
    public void idCardVerify(final Handler mHandler, final FingerprintScanner mScanner, final ParseSFZAPI.People people) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                do {
                    Result res = mScanner.capture();//采集一帧FingerprintImage类型指纹图像
                ...
                Result res = Bione.extractFeature(fi);//从FingerprintImage指纹图像提取特征
                ...
                Result oneResult = Bione.idcardVerify(one, fpTemp);//比对身份证指纹和设备取到的指纹

                Result twoResult = Bione.idcardVerify(two, fpTemp);//比对身份证指纹和设备取到的指纹
